...
void CGuiSampleDoc::OnImageGrabbed( Pylon::CInstantCamera& camera, const Pylon::CGrabResultPtr& grabResult )
{
    ...
    // You must not access any UI objects, since we are not in the GUI thread.
    // At the end of this fuction we will post a message to the main window.
    // In response to this message, the GUI thread we will update the GUI.
...
void CGuiSampleDoc::OnGrabStopped( Pylon::CInstantCamera& camera )
{
    ...
    m_camera.DeregisterConfiguration( &m_continuousConfiguration );
    UNUSED_ALWAYS( camera );
}
...
bool IsSingleShotSupported( Pylon::CInstantCamera &camera )
{
    if (!camera.IsOpen())
    {
        return false;
    ...
    bool canSet = Pylon::CEnumParameter( camera.GetNodeMap(), "AcquisitionMode" ).CanSetValue( "SingleFrame" );
    return canSet;
}
...
{
    pCmdUI->Enable( m_camera.IsOpen() && !m_camera.IsGrabbing() && IsSingleShotSupported( m_camera ) );
}
...
void CGuiSampleDoc::OnStartGrabbing()
{
    ...
    // The predefined configurations are only executed once when the camera is opened.
    // To be able to use them in our use case, we just call them explicitly to apply the configuration.
    m_continuousConfiguration.ApplyConfiguration( m_camera.GetNodeMap() );
