...
def main():
    ...
    stitch_image_array=[]
    result,stitch=process[2](result,4,1)
    ...
    cv.imwrite(path+'11. contours.png',contourImage)
    cv.putText(final_result,'final result',(5,30),cv.FONT_HERSHEY_SIMPLEX,caption_size,(0,0,255),2)
    cv.imwrite(path+'12. final result.png',final_result)
...
def smoothen_image(result,step,enable):#filter noise
    ...
    cv.imwrite(path+str(step)+'. medianBlur_'+str(enable)+'.png',result_medianBlur)
...
def sharpen_image(result,step,enable):#image sharpening
    ...
    cv.imwrite(path+str(step)+'. filter2D_'+str(enable)+'.png',result_filter2D)
...
def erode_image(result,step,enable):#erode
    ...
    cv.imwrite(path+str(step)+'. erode_'+str(enable)+'.png',result_erode)
...
def dilate_image(result,step,enable):#dilate
    ...
    cv.imwrite(path+str(step)+'. dilate_'+str(enable)+'.png',result_dilate)
...
def laplacian(result,step,enable):#laplacian
    ...
    cv.putText(result_laplacian,'Laplacian enabled' if enable else 'Laplacian disabled',(5,30),cv.FONT_HERSHEY_SIMPLEX,caption_size,(0,0,255),2)
    cv.imwrite(path+str(step)+'. Laplacian_'+str(enable)+'.png',result_laplacian)
...
def convert_binary(result,step,enable):#change to black & white
    ...
    cv.imwrite(path+str(step)+'. threshold_'+str(enable)+'.png',result_threshold)
...
def detect_edge(result,step,enable):#edge detection
    ...
    cv.imwrite(path+str(step)+'. Canny_'+str(enable)+'.png',result_Canny)
